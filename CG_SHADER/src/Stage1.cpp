...
void Stage1::Init()
{
	...
	_second_monitor = new Model("res/models/second_monitor.obj");
	{
		Model* model = new Model("res/models/mask2.obj");
		mask2 = new Mask(*model);
		BoxCollider* ptr = new BoxCollider;
		mask2->AddComponent(ptr);
	...
	}
	{
		...
		Model* model2 = new Model("res/models/exitdoor_event.obj");
		int num[] = { 1,2,1,5 };
		exitdoor_rocked = new Locked(*model2, num);
		BoxCollider* ptr2 = new BoxCollider;
		exitdoor_rocked->AddComponent(ptr2);
...

	{
		...
		billboard->SetPosition(150.0f, 20.0f, -560.0f);
	}
...
void Stage1::Object_Render()
{
	...
	shader->SetUniform1i("u_texture", merry_texture->GetSlot());
	for (int i = 0; i < end_room.size(); i++)
	{
	...
	{
		shader->SetUniform1i("u_texture", answer2_texture->GetSlot());
		shader->SetUniformMat4f("u_model", glm::mat4(1.0f));
		answerbox2->Render(*shader);
	}

	{
		shader->SetUniform1i("u_texture", answer3_texture->GetSlot());
		shader->SetUniformMat4f("u_model", glm::mat4(1.0f));
	...
	{
		shader->SetUniform1i("u_texture", answer4_texture->GetSlot());
		shader->SetUniformMat4f("u_model", glm::mat4(1.0f));
...

	{
		if (end->_coding == true)
		{
			shader->SetUniform1i("u_texture", start_texture->GetSlot());
		...
		if (end->dt > 8.5f)
		{
			...
			mask2->SetTransPose(end->ft,0.0f,0.0f);
			shader->SetUniform1i("u_texture", mask_texture->GetSlot());
			mask2->Render(*shader);
...
void Stage1::Object_Render2()
{
	if (mask2->realreal_end == true)
	{
		...
		shader2->SetUniform1i("u_texture", real_end_texture->GetSlot());
		render_box3->RenderModel(*shader2);
		SoundManager::GetInstance()->Stop(HEARTBEAT);
...
void Stage1::Texture_Render()
{
	...
	{
		TextManager::GetInstance()->Render(-0.1f, 0.2f, "THIS DOOR IS LOCKED");
	}

	if (exitdoor_rocked->_collusion)
	{
		TextManager::GetInstance()->Render(-0.1f, 0.0f, "Need a PassWord");
		TextManager::GetInstance()->Render(-0.1f, -0.1f, "Press 1,2,3,4");

		string strValue = std::to_string(exitdoor_rocked->_first_set[0]);
		strValue += std::to_string(exitdoor_rocked->_first_set[1]);
		strValue += std::to_string(exitdoor_rocked->_first_set[2]);
		strValue += std::to_string(exitdoor_rocked->_first_set[3]);

		TextManager::GetInstance()->Render(-0.1f, -0.2f, strValue.c_str());
...
void Stage1::MakeTexture()
{
	...
	billboard_texture = new Texture("res/textures/billboard_test.jpg");
	...
	start_texture = new Texture("res/textures/start_scene.jpg");
